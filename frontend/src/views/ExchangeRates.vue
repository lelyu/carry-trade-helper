...
<template>
  <div class="h-screen flex flex-col bg-gray-50 dark:bg-gray-900">
          ...
          <div class="flex gap-2">
            ...
            <div class="relative">
              <button
                ...
                class="px-3 py-2 text-sm font-medium bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-gray-100 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors"
              >
                Base: {{ store.exchangeBase }}
              </button>
              <div v-if="showBaseSwitcher" class="absolute right-0 top-full mt-1 bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg shadow-lg z-50 py-1 w-24">
                <button
                  v-for="base in BASE_OPTIONS"
                  :key="base"
                  @click="changeBase(base)"
                  class="w-full px-3 py-1.5 text-left text-sm hover:bg-gray-50 dark:hover:bg-gray-600"
                  :class="base === store.exchangeBase ? 'text-blue-600 dark:text-blue-400 font-medium' : 'text-gray-700 dark:text-gray-300'"
                >
                  {{ base }}
                </button>
              </div>
    ...
    <main class="flex-1 overflow-y-auto px-4 pt-3 pb-20 md:pb-6">
      ...
      <div v-else-if="store.exchangeError" class="mt-4 p-4 bg-red-50 dark:bg-red-900/20 rounded-lg text-red-700 dark:text-red-400 text-sm">
        {{ store.exchangeError }}
      </div>
        ...
        >
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-3">
              <div>
                <div class="font-semibold text-gray-900 dark:text-gray-100">{{ rate.target_currency }}</div>
                <div class="text-xs text-gray-400 dark:text-gray-500">{{ CURRENCY_NAMES[rate.target_currency] || '' }}</div>
              </div>
            </div>

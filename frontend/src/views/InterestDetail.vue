...
const currentRate = computed(() =>
  store.interestRates.find(r => r.country_code === props.code)
)
...
const COUNTRY_NAMES: Record<string, string> = {
  USA: 'United States', EUR: 'Euro Area', GBR: 'United Kingdom',
  JPN: 'Japan', CHE: 'Switzerland', AUD: 'Australia',
  CAN: 'Canada', NZL: 'New Zealand', CHN: 'China', HKG: 'Hong Kong',
}
...
const fetchHistoricalData = async (period: Period) => {
  ...
  const data = await store.fetchHistoricalInterestRates(props.code, from, to)
...
onMounted(() => {
  if (store.interestHistory7d && store.interestHistory7d[props.code] && store.interestHistory7d[props.code].length > 0) {
    chartData.value = store.interestHistory7d[props.code].map(p => ({
      date: p.date,
      rate: Number(p.rate),
    }))
  } else {
    ...
    <header class="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 shrink-0 z-40">
      <div class="px-4 py-3">
        <div class="flex items-center justify-between">
          <router-link to="/interest" class="text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300 transition-colors">
    ...
    <main class="flex-1 overflow-y-auto px-4 pt-4 pb-20 md:pb-6">
      <div v-if="currentRate" class="space-y-4">
        <div class="bg-white dark:bg-gray-800 rounded-lg p-5">
          <div class="text-sm text-gray-400 dark:text-gray-500">{{ currentRate.country_code }} &middot; {{ currentRate.currency_code }}</div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg p-4">
          ...
          <div v-if="chartData.length > 0" class="w-full">
            <LineChart :data="chartData" color="#10b981" :height="250" :step="true" />
          </div>

    ...
    async with async_session_maker() as db:
        try:
            exchange_data = await frankfurter_client.get_latest_rates(
                base="USD",
                quotes=settings.SUPPORTED_CURRENCIES
            )
            ...
            interest_data = await dbnomics_client.get_interest_rates(
                country_codes=["USA", "EUR", "GBR", "JPN", "CHF", "AUD", "CAD", "NZD", "CHN", "HKG"]
            )
            ...
            await db.commit()
            return {"status": "success", "message": "Rates fetched and cached successfully"}
